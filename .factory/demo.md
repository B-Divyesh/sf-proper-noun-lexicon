...
- URL: <https://proper-noun-lexicon.sociobot.in/demo>
...
The command embeds `cli/examples/sample-terms.csv` and `cli/examples/raw-transcript.txt`. It creates a unique directory under the operating system's temporary directory, then writes the sample CSV, raw text, portable lexicon, corrected text, rollback audit, Whisper prompt, Google PhraseSet, and Azure phrase list. It prints the exact directory and never writes to the current project.
