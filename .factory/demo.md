...
- Isolation: demo state uses only local-storage keys beginning with `demo:pnl:`. Its workspace record holds the sample vocabulary, raw transcript, and latest audit. It never reads the real `pnl:workspace:v1` key and does not initialize license verification.
- Reset: **Reset demo** restores the bundled three-term vocabulary and raw transcript, then clears the sample audit.
